...
- **OpenSSH is non-root:** `ssh_setup.sh` sets `PermitRootLogin no` and installs the gitignored live `scripts/authorized_keys` only for the non-root `user` account. Routine admin goes through `sudo`; first-time password setup or root recovery uses `adb shell su` or Tailscale identity SSH where ACLs allow it.
