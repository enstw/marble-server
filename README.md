...
- `scripts/` — shell scripts that run on the device
  ...
  - `android.sh` — `android lock` / `android unlock [--stayon]`: manual screen-off / wake over the chroot-escape path (counter to screen-off CPU throttling; see `docs/MAINTENANCE.md` §1). Also `android beep` / `android play <file>`: audible alerts via Termux:API MediaPlayer (one-time prereqs: Termux:API APK + `termux-api` pkg + `allow-external-apps=true`); and `android volume up|down [n]`: adjust the active audio stream
  - `reboot.sh` — root helper deployed at `/usr/local/sbin/reboot`; schedules detached reboot then SIGHUPs the per-session sshd for a clean disconnect (self-elevates via sudo when `user` calls it bare; NOPASSWD)
